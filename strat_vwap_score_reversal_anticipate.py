        ...
        if not active_position:
            ...
            if pd.notna(prev_score) and pd.notna(score):
                ...
                if prev_score < CLENOW_THRESHOLD and score >= CLENOW_THRESHOLD:
                    ...
                    print(f"[ENTRY SHORT ANTICIPATE] {current_time} @ {entry_price:.2f} (Green Dot: {prev_score:.2f} -> {score:.2f} >= +{CLENOW_THRESHOLD}) | TP: {tp_price:.2f} | SL: {sl_price:.2f}")
                ...
                # LONG ENTRY: Score crosses DOWN through -Threshold (Red Dot - Immediate)
                    ...
                    print(f"[ENTRY LONG ANTICIPATE] {current_time} @ {entry_price:.2f} (Red Dot: {prev_score:.2f} -> {score:.2f} <= -{CLENOW_THRESHOLD}) | TP: {tp_price:.2f} | SL: {sl_price:.2f}")
